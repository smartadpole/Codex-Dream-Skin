...
if [ "$verify_code" -ne 0 ]; then
  ...
  # The watcher is managed through launchd, but launchd state alone does not
  # prove that the recorded process stopped. Verify the PID/path/start-time
  # tuple before deleting state; if it cannot be stopped safely, preserve the
  # state as evidence and fail closed instead of leaving an orphan watcher that
  # can reinject later.
