...
remove_injector_launchd_job() {
  /bin/launchctl bootout "gui/$(/usr/bin/id -u)/$INJECTOR_JOB_LABEL" >/dev/null 2>&1 || true
...
write_injector_launch_agent() {
  ...
  "$NODE" -e '
    const fs = require("node:fs");
    const [file, label, nodePath, injectorPath, port, themeDir, stdoutPath, stderrPath] = process.argv.slice(1);
    const esc = (value) => String(value)
      ...
      .replaceAll("\"", "&quot;")
      .replaceAll("'\''", "&apos;");
    const string = (value) => `<string>${esc(value)}</string>`;
...
`;
  ...
  ' "$INJECTOR_PLIST" "$INJECTOR_JOB_LABEL" "$NODE" "$INJECTOR" "$port" "$THEME_DIR" "$INJECTOR_LOG" "$INJECTOR_ERROR_LOG"
...
launch_injector_daemon() {
  ...
  # Launch the resident watcher through a controlled LaunchAgent plist so it
  # survives the starter shell without becoming an unmanaged external launcher.
  # The fixed job label is intentionally booted out before each start; this is
  # one watcher daemon with one state file.
  write_injector_launch_agent "$port"
  /bin/launchctl bootstrap "gui/$(/usr/bin/id -u)" "$INJECTOR_PLIST" >/dev/null 2>&1 || true
  ...
  while [ "$SECONDS" -lt "$deadline" ]; do
    pid="$(/bin/launchctl print "gui/$(/usr/bin/id -u)/$INJECTOR_JOB_LABEL" 2>/dev/null \
      | /usr/bin/awk '/^[[:space:]]*pid = [0-9]+/ && !found { pid=$3; found=1 } END { if (found) print pid }')"
    if [ -n "$pid" ] && /bin/kill -0 "$pid" 2>/dev/null; then
      printf '%s\n' "$pid"
      return 0
    fi
    # Also detect the managed node process by exact command line because
    # launchctl output varies across macOS versions.
    pid="$(/bin/ps -axo pid=,command= | /usr/bin/awk -v node="$NODE" -v inj="$INJECTOR" -v port="$port" '
      !found && index($0, node) && index($0, inj) && index($0, "--watch") && index($0, "--port " port " --theme-dir ") { print $1; found=1 }
    ')"
    if [ -n "$pid" ] && /bin/kill -0 "$pid" 2>/dev/null; then
      printf '%s\n' "$pid"
      return 0
...
hot_reapply_theme() {
  ...
  if [ "$injector_protocol" = "2" ]; then
    inj_pid="$(/bin/ps -axo pid=,command= | /usr/bin/awk -v node="$NODE" -v inj="$INJECTOR" -v port="$port" '
      !found && index($0, node) && index($0, inj) && index($0, "--watch") && index($0, "--port " port " --theme-dir ") { print $1; found=1 }
    ')"
  fi
